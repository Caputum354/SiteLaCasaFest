(function () {
  ...
  /* ==========================================
     LAZY LOAD + AUTOPLAY SÓ DO VÍDEO VISÍVEL
     (evita carregar/tocar os 6+ vídeos ao
     mesmo tempo — principal gargalo no mobile)
     ========================================== */

  function carregarVideo(video) {
    if (!video || video.src) return;

    const origem = video.dataset.src;
  ...
  if ("IntersectionObserver" in window) {
    ...
    const observadorVideo = new IntersectionObserver(
      function (entradas) {
        ...
        entradas.forEach(function (entrada) {
          ...
          const video =
            entrada.target.querySelector("video");
    ...
    /*
     * Sempre que o carrossel é remedido
     * (clones novos após resize), volta a
     * observar todos os posts atuais.
     */
    const medirOriginal = medir;
    ...
    /*
     * Sem suporte a IntersectionObserver:
     * carrega tudo sob demanda no hover,
     * sem autoplay automático.
     */
    trilho
      .querySelectorAll("video")
      .forEach(carregarVideo);
  }
  ...
  feed.addEventListener(
    ...
    function (evento) {
       ...
       * (carrega sob demanda, se preciso)
       */
      tocarVideo(card.querySelector("video"));
    }
  );
  ...
  feed.addEventListener(
    ...
    function (evento) {
      ...
      /*
       * PARA O VÍDEO
       * (só reseta o tempo se o card não
       * estiver visível no carrossel, para não
       * conflitar com o autoplay por scroll)
       */
      const video =
